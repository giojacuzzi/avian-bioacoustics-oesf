...
for (file in files) {
  wav = readWave(file)
  ndsi_soundecology = ndsi(wav)$ndsi_left
  ndsi_seewave = NDSI(soundscapespec(wav, plot=F))
  results = rbind(results, data.frame(soundecology=ndsi_soundecology, seewave=ndsi_seewave))
}
```

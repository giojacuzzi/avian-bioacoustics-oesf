...
def find_files_by_date(directory, year, month, day):
    pattern = re.compile(filename_convention)
    all_files = find_wav_files(directory)
    # print(all_files)
...
# Uses a predefined subsampling period interval scheme to select subsample start times randomly from the deployment period dates
def get_subsample_datetimes(id, lat, lon, year, month_start, month_end, day_start, day_end):

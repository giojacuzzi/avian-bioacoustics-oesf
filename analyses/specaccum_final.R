#### lean_5d15s_12d12s proved to provide the highest number of unique species encounters (up to 1/3 of the site) with a minimal amount of annotation effort (roughly 30 minutes per site)
...
sunrise_time = as.POSIXct("2020-06-10 05:20:00")
sunset_time = as.POSIXct("2020-06-10 21:13:00")
...
by_dense = '5 min'
by_sparse = '15 min'
window_dense = 15 # sec per sample
window_sparse = 15 # sec per sample
full_5d15s_15d15s = ref[ref$start_date %in% c(
  # Sparse from midnight to hour before sunrise
  get_times_by_interval_window(ref$start_date,
                               t_start = as.POSIXct("2020-06-10 00:00:00"),
                               t_end = sunrise_time - 3600*1,
                               by = by_sparse, window = window_sparse),
  # Dense from 1hr before sunrise to 6hr after
  get_times_by_interval_window(ref$start_date,
                               t_start = sunrise_time - 3600*1,
                               t_end = sunrise_time + 3600*6,
                               by = by_dense, window = window_dense),
  # Sparse from 6hr after sunrise to 5hr before sunset
  get_times_by_interval_window(ref$start_date,
                               t_start = sunrise_time + 3600*6,
                               t_end = sunset_time - 3600*5,
                               by = by_sparse, window = window_sparse),
  # Dense from 5 hours before sunset to sunset
  get_times_by_interval_window(ref$start_date,
                               t_start = sunset_time - 3600*5,
                               t_end = sunset_time,
                               by = by_dense, window = window_dense),
  # Sparse from sunset to midnight
  get_times_by_interval_window(ref$start_date,
                               t_start = sunset_time,
                               t_end = as.POSIXct("2020-06-10 23:59:59"),
                               by = by_sparse, window = window_sparse)
), ]
...
by_dense = '5 min'
by_sparse = '15 min'
window_dense = 12 # sec per sample
window_sparse = 12 # sec per sample
lean_5d15s_12d12s = ref[ref$start_date %in% c(
  # Sparse from midnight to hour before sunrise
  get_times_by_interval_window(ref$start_date,
                               t_start = as.POSIXct("2020-06-10 00:00:00"),
                               t_end = sunrise_time - 3600*1,
                               by = by_sparse, window = window_sparse),
  # Dense from 1hr before sunrise to 4hr after
  get_times_by_interval_window(ref$start_date,
                               t_start = sunrise_time - 3600*1,
                               t_end = sunrise_time + 3600*4,
                               by = by_dense, window = window_dense),
  # Sparse from 4hr after sunrise to 4hr before sunset
  get_times_by_interval_window(ref$start_date,
                               t_start = sunrise_time + 3600*4,
                               t_end = sunset_time - 3600*4,
                               by = by_sparse, window = window_sparse),
  # Dense from 4 hours before sunset to sunset
  get_times_by_interval_window(ref$start_date,
                               t_start = sunset_time - 3600*4,
                               t_end = sunset_time,
                               by = by_dense, window = window_dense),
  # Sparse from sunset to midnight
  get_times_by_interval_window(ref$start_date,
...
window_dense = 12 # sec per sample
window_sparse = 12 # sec per sample
lean_double_by = ref[ref$start_date %in% c(
  # Sparse from midnight to hour before sunrise
  get_times_by_interval_window(ref$start_date,
                               t_start = as.POSIXct("2020-06-10 00:00:00"),
                               t_end = sunrise_time - 3600*1,
...
by_dense = '5 min'
by_sparse = '15 min'
window_dense = 24 # sec per sample
window_sparse = 24 # sec per sample
lean_double_window = ref[ref$start_date %in% c(
  # Sparse from midnight to hour before sunrise
  get_times_by_interval_window(ref$start_date,
                               t_start = as.POSIXct("2020-06-10 00:00:00"),
                               t_end = sunrise_time - 3600*1,
...
window_dense = 12 # sec per sample
window_sparse = 12 # sec per sample
lean_3d20s_12d12s = ref[ref$start_date %in% c(
  # Sparse from midnight to hour before sunrise
  get_times_by_interval_window(ref$start_date,
                               t_start = as.POSIXct("2020-06-10 00:00:00"),
                               t_end = sunrise_time - 3600*1,
...
by_dense = '6 min'
by_sparse = '20 min'
window_dense = 30 # sec per sample
window_sparse = 30 # sec per sample
lean_6d20s_30d30s = ref[ref$start_date %in% c(
  # Sparse from midnight to hour before sunrise
  get_times_by_interval_window(ref$start_date,
                               t_start = as.POSIXct("2020-06-10 00:00:00"),
                               t_end = sunrise_time - 3600*1,
...
schemes = list(
  ...
  # cont_1d4 = cont_1d4,
  # cont_1d5 = cont_1d5,
  # full_5d15s_15d15s = full_5d15s_15d15s,
  lean_5d15s_12d12s = lean_5d15s_12d12s, # THIS IS OUR ONE!
  lean_3d20s_12d12s = lean_3d20s_12d12s,
  lean_double_window = lean_double_window,
  lean_double_by = lean_double_by
  # lean_6d20s_30d30s = lean_6d20s_30d30s
)
...
ggplot() +
  geom_line(df_compare, mapping = aes(x = samples, y = richness, group = scheme, color = scheme, linetype = scheme), alpha = 0.5, linewidth = 1) +

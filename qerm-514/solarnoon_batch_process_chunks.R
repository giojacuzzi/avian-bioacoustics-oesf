...
for (date in as.character(unique(data$SurveyDate))) {
  ...
  for (serialno in unique(date_data$SerialNo)) {
    ...
    if (sum(solarNoon_files %in% files_processed$File) != 0) {
...
message('Processing ', length(files_to_process))
message('NOTE: ', n_files_already_processed, ' files already processed')
...
for (date in as.character(sort(unique(as.Date(files_to_process$SurveyDate))))) {
  ...
  solarNoon_files = files_to_process[files_to_process$SurveyDate==date, 'File']
  ...
  message(' SerialNos ', paste(unique(data[data$SurveyDate==date, 'SerialNo']), collapse = ' '))

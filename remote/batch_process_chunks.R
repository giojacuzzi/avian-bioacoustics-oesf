...
# Only look at 2021 for now
data = data[data$DataYear==2021, ]
data = data[!is.na(data$File),]

output_path = paste0(dirname(getSourceEditorContext()$path), '/output')
...
for (date in as.character(sort(unique(as.Date(files_to_process$SurveyDate))))) {
  ...
  for (batch in batches) {
    message(' SerialNos ', paste(unique(get_serial_from_file_name(batch)), collapse = ' '))
    
    alpha_indices = c('ACI', 'BIO', 'ADI', 'AEI', 'NDSI', 'H', 'M')
    process_data = batch_process_acidx_df(
      batch,
    ...
    diagnostics$SerialNo = get_serial_from_file_name(diagnostics$File)
